...
- If a release task (test, build, etc.) fails, STOP. Do not attempt to fix
  the underlying bug. Log the failure, mark the task as `in-progress`, and
  report what went wrong. Fixing bugs is a separate task from releasing.
- Update the `.mrc` instructions and scripts when you discover mistakes or
  missing steps, so the next run is better.


## A new release with mulle-release-commander

**You may be asked to do a "release". Don't do it now, just do it on request.**

There is a `.mulle/etc/release-commander/mulle-gdb.mrc` mulle-release-commander
prokect file. It is part of the repository and will be committed alongside,
we can edit this at will.

It contains a `AGENTS.md` file.

You follow the instructions and then you should have a new release ready. If
you notice mistakes or need additional steps, you updat the .mrc alongside with
it so we are off to a better start next time.
